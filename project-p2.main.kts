...
// would be surprising), say that.
    ...
    // I was partners with Ahana Guin.
...
class TaggedFlashCard(
    ...
    fun fileFormat(): String {
        var tagString = ""
        if (tags.isNot()) {
        tagString = tags.joinToString(separator = ",")
        }
        return "$front|$back|$tagString"
    }
}

@EnabledTest
fun testTaggedFlashCard() {
    val exampleCard1 = TaggedFlashCard("What is Barbie's full name?", "Barbara Millicent Roberts", listOf("Random Trivia", "easy"))
    val exampleCard2 = TaggedFlashCard("What spice is a hallucinogen?", "Nutmeg", listOf("Food Trivia", "easy/medium"))
    val exampleCard3 = TaggedFlashCard("How fast is light in miles per second?", "186,282 miles per second", listOf("Science Trivia", "hard"))

    testSame(exampleCard1.isTagged("Random Trivia"), true, "Card 1 is tagged Random Trivia")
    testSame(exampleCard2.isTagged("Math Trivia"), false, "Card 2 is not tagged Math Trivia")

    testSame(
        exampleCard1.fileFormat(), 
        "What is Barbie's full name?|Barbara Millicent Roberts|Random Trivia,easy",
        "Card 1 has the right file format"
        )
    testSame(
        exampleCard3.fileFormat(), 
        "How fast is light in miles per second?|186,282 miles per second|"Science Trivia", "hard",
        "Card 2 has the right file format"
        )
}
...
//
fun stringToTaggedFlashCard(input: String): TaggedFlashCard?{
    val parts = input.split(sepCard)
    if (parts.size != 3) {
        return null
    }
    val front = parts[0]
...
fun readTaggedFlashCardsFile(filePath: String): List<TaggedFlashCard?> {
    ...
    return file.readLines().mapNotNull { stringToTaggedFlashCard(it) }
}


@EnabledTest
fun testReadTaggedFlashCardsFile() {
    val existingFilePath = "example_tagged.txt"
    val existingCards = readTaggedFlashCardsFile(existingFilePath)
    
    testSame(existingCards.size, 2, "Number of cards in existing file")

    val nonExistentFilePath = "non_existent_file.txt"
    val nonExistentCards = readTaggedFlashCardsFile(nonExistentFilePath)

    testSame(nonExistentCards.size, 0, "Number of cards in non-existent file")
}
// -----------------------------------------------------------------
// Deck design
// -----------------------------------------------------------------
